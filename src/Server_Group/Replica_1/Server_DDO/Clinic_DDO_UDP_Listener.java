...
public class Clinic_DDO_UDP_Listener implements Runnable{
	...
	public void run() {
		...
		try{
			socket = new DatagramSocket(Server_DDO_Config.LOCAL_LISTENING_PORT);
			while(true){
				byte[] buffer = new byte[1000]; 
				DatagramPacket request = new DatagramPacket(buffer, buffer.length);
				socket.receive(request);
				Server_DDO_Config.LOGGER.info("Get request: " + (new String(request.getData()).trim())+ "\n" + "Start a new thread to handle this.");
	...
	static class Connection extends Thread{
		...
		public Connection(DatagramSocket n_socket, DatagramPacket n_request) {
			...
			switch (requestcode) {
			case "002":
				Server_DDO_Config.LOGGER.info("Request code: " + requestcode + ", " + "Search HashMap, SearchType: " + (new String(request.getData()).trim().substring(4)));
				result = getLocalHashSize(new String(request.getData()).trim().substring(4));
				break;
			case "003":
				Server_DDO_Config.LOGGER.info("Request code: " + requestcode + ", " + "Transfer Record, Record: " + (new String(request.getData()).trim().substring(4)));
	...
	public static synchronized String insertRecordInLocalHashMap(String recordInfo){
		...
		if(record[0].contains("DR")){
			if(Server_DDO_Config.HASH_TABLE.containsKey(record[2].split(": ")[1].charAt(0))){
				Server_DDO_Config.RECORD_LIST = Server_DDO_Config.HASH_TABLE.get(record[2].split(": ")[1].charAt(0));
			}else{
