...
public class Front_End_Listener_Thread implements Runnable {
    ...
    public static Boolean Broad_Cast_Request(String request_ID, InetAddress address, Integer port) {
        ...
        try {
            ...
            for (Entry<Integer, String> entry : Front_End_Config.liveHostsByName.entrySet()) {
                if (entry.getKey() == Server_Group.Replica_1.UDP_Replica_Manager.Replica_Manager_Config.LOCAL_BROAD_CAST_LISTENING_PORT) {
                    ...
                    do {
                        ...
                        String message = Server_Group.Replica_1.UDP_Replica_Manager.Replica_Manager_Config.REQUEST_HASH_TABLE.get(currentRequestId);
                        ...
                        try {
                            ...
                            if (Server_Group.Replica_1.UDP_Replica_Manager.Replica_Manager_Config.REQUEST_HASH_TABLE.containsKey(currentRequestId)) {
                            ...
                            return Broad_Cast_Request(request_ID, address, port);
                        }
                    } while (retransfer);
    ...
    public void run() {
        DatagramSocket socket = null;
        try {
            socket = new DatagramSocket(Server_Group.Replica_1.UDP_Replica_Manager.Replica_Manager_Config.LOCAL_FRONT_END_LISTENING_PORT); // port: 4000
            while (true) {
                ...
                if (!request_ID.equals("0000")) {
                    ...
                    Server_Group.Replica_1.UDP_Replica_Manager.Replica_Manager_Config.REQUEST_HASH_TABLE.put(request_ID, new String(request.getData()).trim());
