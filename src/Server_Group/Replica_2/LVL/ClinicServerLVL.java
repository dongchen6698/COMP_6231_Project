...
import org.omg.PortableServer.POAHelper;
	...
	public class ClinicServerLVL extends ClinicServerIntPOA implements Runnable{
		private ORB orb;
		...
		public ClinicServerLVL(String[] arg)
		{
			this.args = arg;
		}
		...
		public void run() {
		    ...
		    try {
				...
				//ClinicServerLVL CS = new ClinicServerLVL();
				setORB(orb);
		...
		public void start() {
		    ...
		    System.out.println("Thread LVL Starting!");
		    if (t == null)
		        t = new Thread(this, "LVL");
		    t.start();
		...
		public String getRecordCounts (String managerID, String recordType)
		{
			...
			return "LVL " + count + " ," + sendReq("get","MTL","","",recordType) + " ," + sendReq("get","DDO","","",recordType);
		}
		...
		public String transferRecord(String managerID,String recordID, String remoteClinicServerName){
			if(DetailTable.containsKey(recordID))
			{
				...
				String result = "The record for "+recordID+" does not exist in LVL Server";
				ServerLog("transferRecord", result);			
				return result;
			}
		}
		...
		private void ServerLog(String Method, String Description)
		{
			...
			try(FileWriter fw = new FileWriter("Server_Side_Log/host_2_lvl.txt", true);
		...
		private void listener(){
			DatagramSocket aSocket = null;
			try{
				aSocket = new DatagramSocket(LVL_UDPPort);
				byte[] buffer = new byte[1000];
				while(true){
