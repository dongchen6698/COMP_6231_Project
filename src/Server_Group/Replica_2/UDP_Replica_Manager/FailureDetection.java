...
public class FailureDetection extends TimerTask {
    ...
    public FailureDetection() {
        ...
        Front_End_Config.liveHostsByName.put(5100, "Host_2");

        timer.scheduleAtFixedRate(this, Replica_Manager_Config.INITIALDELAY, Replica_Manager_Config.INTERVAL);

        int newLeaderIndex = elect(Replica_Manager_Config.REPLICA[1]);
    ...
    public String doPing(String targetAddress, int targetPort, String msg) {
        ...
        try {
            ...
            Front_End_Config.liveHostsByName.remove(5100);
            ...
            public void run() {
                try {
                    ...
                    while (true) {
                        ...
                        System.out.println("Replica number two replied");
                        logger.info("Replica number two replied");
    ...
    int elect(int initiator) {
        ...
        for (int i = 0; i < Replica_Manager_Config.n; i++) {
            if (Replica_Manager_Config.priority[initiator] < Replica_Manager_Config.priority[i]) {
                ...
                if (i + 1 < Server_Group.Replica_2.UDP_Replica_Manager.Replica_Manager_Config.n && doPing(Replica_Manager_Config.HOST_NAME, Replica_Manager_Config.priority[Replica_Manager_Config.REPLICA[i + 1]], "0000" + "\n" + "006" + "\n" + "0000000") != null)
                    elect(i + 1);
            }
        }
