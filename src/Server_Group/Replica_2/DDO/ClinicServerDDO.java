...
public class ClinicServerDDO extends ClinicServerIntPOA implements Runnable{
		...
		public ClinicServerDDO(String[] arg)
		{
			this.args = arg;
		}
		...
		public void start() {
		    ...
		    System.out.println("Thread DDO Starting!");
		    if (t == null)
		        t = new Thread(this, "DDO");
		    t.start();
		...
		public String getRecordCounts (String managerID, String recordType)
		{
			...
			return "DDO " + count + " ," + sendReq("get","MTL","","",recordType) + " ," + sendReq("get","LVL","","",recordType);
		}
		...
		public String transferRecord(String managerID, String recordID, String remoteClinicServerName){
			...
			{				
				...
				String result = "The record for "+recordID+" does not exist in DDO Server";
				ServerLog("transferRecord", result);			
				return result;
			}
		}
		...
		private void ServerLog(String Method, String Description)
		{
			...
			try(FileWriter fw = new FileWriter("Sserver_Side_Log/host_2_ddo.txt", true);
		...
		private String sendReq(String type, String server, String managerID, String recordID, String RecordType)
		{
			...
			case "trs":
				try{
					...
					aSocket.receive(reply);
					ServerLog("sendReq : transfer :","Server received a reply");
					result = new String(reply.getData()).substring(0, reply.getLength());
		...
		private void listener(){
			DatagramSocket aSocket = null;
			try{
				aSocket = new DatagramSocket(DDO_UDPPort);
				byte[] buffer = new byte[1000];
				while(true){
